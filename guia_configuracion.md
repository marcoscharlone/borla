...
A: SKU · B: Nombre · C: Categoría (se muestra en el catálogo como "Pasillo/Sector") · D: Proveedor · E: Unidad · F: Precio Costo · G: Precio Venta · H: Margen % (fórmula, no se muestra) · I: Stock Actual · J: Stock Mínimo · K: Estado Stock (fórmula, no se muestra pero se usa para el filtro "solo stock bajo" y resalta la fila) · L: Última Actualización · M: Código EAN · N: Código Proveedor · **P: Bloque**
...
- **Editar el Markup directamente:** la columna Markup del catálogo también es editable. Si escribís un nuevo Markup, la Venta se recalcula sola (Costo × (1 + Markup%)) — y si en cambio editás la Venta (o el Costo), el Markup mostrado se recalcula y recolorea al toque. En los dos casos, lo único que se guarda en la hoja al hacer clic en **Guardar** es el precio de Venta resultante; el Markup nunca se guarda como tal, siempre se calcula a partir de Costo y Venta.
...
- **Filtrar:** por nombre/SKU/EAN, pasillo/sector (categoría), proveedor, bloque, o solo productos con stock bajo.
...
- **Actualización masiva de precio de venta por proveedor:** en el panel correspondiente, elegí el proveedor y el markup deseado (por ejemplo `35` para 35%). Calcula el nuevo precio de venta como Costo × (1 + markup%) para cada producto de ese proveedor. Los productos sin costo cargado quedan afuera automáticamente. Igual que las demás actualizaciones masivas, primero previsualiza los cambios (podés destildar productos puntuales) y recién después de confirmar los aplica y los registra en el historial.
