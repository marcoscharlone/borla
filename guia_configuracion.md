...
- **Actualizar costo desde factura (otros proveedores):** para cualquier proveedor que no sea Borla Hnos ni VCC 365 (esos dos ya tienen su botón dedicado arriba), usá el cuadro "Actualizar costo desde factura (otros proveedores)":
  ...
  2. Conseguí una **foto de la factura**, de dos formas: con el botón **"📷 Usar cámara"** (abre la cámara del dispositivo dentro de la misma página, pedí permiso la primera vez, y "Capturar foto" saca la imagen), o eligiendo un archivo ya existente con el selector de "Foto de la factura". Queda mostrada en pantalla.
  3. Opcional: hacé clic en **"🔎 Leer factura automáticamente"** para que la app intente detectar solo los códigos y precios de la foto y precargue la tabla de líneas. Es una lectura automática (OCR) que corre en el propio navegador, sin mandar la foto a ningún servicio externo — pero **no es confiable**: con fotos reales (letra chica, torcidas, distintos formatos según el proveedor) va a fallar seguido, confundir código con cantidad, o directamente no reconocer algunas líneas. Por eso, después de usarla, **revisá y corregí cada fila contra la foto antes de seguir** — la tabla queda igual de editable que si la hubieras cargado a mano. Si preferís no arriesgarte a errores, salteá este paso y cargá las líneas directamente a mano.
  4. Arriba de la tabla, marcá si el **precio de la factura incluye IVA** (si no lo incluye, completá el % de IVA para que se sume) y si el **precio es por unidad** o por bulto — estas dos condiciones aplican a toda la factura.
  5. Revisá/cargá una fila por ítem: **Código Proveedor** (se cruza contra la columna N del catálogo, igual que Borla/VCC), la **Descripción** es solo para tu referencia visual y no se guarda, el **Precio** tal cual figura en la factura, la **Cantidad** del bulto (dejalo en 1 si el precio ya es por unidad), y el **% de descuento** de esa línea en particular — a diferencia del IVA y el precio unitario, el descuento sí se carga línea por línea, porque no siempre aplica igual a todos los ítems de una factura. La columna "Costo final" se recalcula sola con cada cambio.
  6. Hacé clic en **"+ Agregar línea"** si necesitás más filas.
  7. Hacé clic en **Previsualizar cambios**, revisá la tabla (podés destildar productos puntuales) y confirmá — igual que las demás actualizaciones masivas, queda registrado en el historial.
